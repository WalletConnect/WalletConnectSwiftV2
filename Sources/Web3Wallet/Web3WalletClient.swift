...
public class Web3WalletClient {
    ...
    public func buildApprovedNamespaces(
        ...
        try requiredNamespaces.forEach {
            ...
            if let proposalChains = proposalNamespace.chains {
                ...
                let sessionNamespace = SessionNamespace(chains: sessionChains, accounts: accounts, methods: sessionMethods, events: sessionEvents)
                ...
                {
                    let sessionChains = Set(
                        [Blockchain(namespace: network, reference: chain)]
                    ).intersection(Set(chains))
                    guard !sessionChains.isEmpty else {
                        throw AutoNamespacesError.requiredChainsNotSatisfied
                    }
        ...
        try optionalNamespaces!.forEach {
            let caip2Namespace = $0.key
            let proposalNamespace = $0.value
            ...
            if let proposalChains = proposalNamespace.chains {
                let sessionChains = Set(proposalChains).intersection(Set(chains))
                guard !sessionChains.isEmpty else {
                    return//throw AutoNamespacesError.requiredChainsNotSatisfied
                }
                
                let sessionMethods = Set(proposalNamespace.methods).intersection(Set(methods))
                guard !sessionMethods.isEmpty else {
                    return//throw AutoNamespacesError.requiredMethodsNotSatisfied
                }
                
                let sessionEvents = Set(proposalNamespace.events).intersection(Set(events))
                guard !sessionEvents.isEmpty else {
                    return//throw AutoNamespacesError.requiredEventsNotSatisfied
                }
