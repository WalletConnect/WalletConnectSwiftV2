...
//        crypto.set(privateKey: privateKey)
        relay.subscribe(topic: proposal.topic)
        let newTopic = agreementKeys.sharedSecret.toHexString().sha256()
        relay.subscribe(topic: newTopic)
        print("shared key: \(agreementKeys.sharedSecret.toHexString())")
