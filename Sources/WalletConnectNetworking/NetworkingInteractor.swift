...
public class NetworkingInteractor: NetworkInteracting {
    ...
    private let requestPublisherSubject = PassthroughSubject<(topic: String, request: RPCRequest, rawRequest: String?, publishedAt: Date, derivedTopic: String?), Never>()
    ...
    public var requestPublisher: AnyPublisher<(topic: String, request: RPCRequest, rawRequest: String?, publishedAt: Date, derivedTopic: String?), Never> {
    ...
    public func requestSubscription<RequestParams: Codable>(on request: ProtocolMethod) -> AnyPublisher<RequestSubscriptionPayload<RequestParams>, Never> {
        return requestPublisher
            ...
            .compactMap { topic, rpcRequest, rawRequest, publishedAt, derivedTopic in
                guard let id = rpcRequest.id, let request = try? rpcRequest.params?.get(RequestParams.self) else { return nil }
                return RequestSubscriptionPayload(id: id, topic: topic, request: request, rawRequest: rawRequest, publishedAt: publishedAt, derivedTopic: derivedTopic)
            }
            .eraseToAnyPublisher()
    }
    ...
    private func manageSubscription(_ topic: String, _ encodedEnvelope: String, _ publishedAt: Date) {
        if let (deserializedJsonRpcRequest, derivedTopic, rawData): (RPCRequest, String?, String?) = serializer.tryDeserialize(topic: topic, encodedEnvelope: encodedEnvelope) {
            handleRequest(topic: topic, request: deserializedJsonRpcRequest, rawRequest: rawData, publishedAt: publishedAt, derivedTopic: derivedTopic)
        } else if let (response, derivedTopic, rawData): (RPCResponse, String?, String?) = serializer.tryDeserialize(topic: topic, encodedEnvelope: encodedEnvelope) {
    ...
    private func handleRequest(topic: String, request: RPCRequest, rawRequest: String?, publishedAt: Date, derivedTopic: String?) {
        do {
            try rpcHistory.set(request, forTopic: topic, emmitedBy: .remote)
            requestPublisherSubject.send((topic, request, rawRequest, publishedAt, derivedTopic))
        } catch {
            logger.debug(error)
        }
    }
