...
public struct SignClientFactory {
    ...
    public static func create(metadata: AppMetadata, relayClient: RelayClient, pairingClient: PairingClient, networkingClient: NetworkingClient) -> SignClient {
        let logger = ConsoleLogger(loggingLevel: .off)
        let keyValueStorage = UserDefaults.standard
        let keychainStorage = KeychainStorage(serviceIdentifier: "com.walletconnect.sdk")
        return SignClientFactory.create(metadata: metadata, logger: logger, keyValueStorage: keyValueStorage, keychainStorage: keychainStorage, relayClient: relayClient, pairingClient: pairingClient, networkingClient: networkingClient)
    }

    static func create(metadata: AppMetadata, logger: ConsoleLogging, keyValueStorage: KeyValueStorage, keychainStorage: KeychainStorageProtocol, relayClient: RelayClient, pairingClient: PairingClient, networkingClient: NetworkingClient) -> SignClient {
