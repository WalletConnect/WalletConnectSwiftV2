...
public final class SignClient {
    ...
    ///
    /// WalletConnect Client is not a singleton but once you create an instance, you should not deinitialize it. Usually only one instance of a client is required in the application.
    public convenience init(metadata: AppMetadata, projectId: String, relayHost: String, socketImplementation: WebSocketConnecting.Type, keyValueStorage: KeyValueStorage = UserDefaults.standard) {
        self.init(metadata: metadata, projectId: projectId, relayHost: relayHost, logger: ConsoleLogger(loggingLevel: .off), kms: KeyManagementService(serviceIdentifier: "com.walletconnect.sdk"), keyValueStorage: keyValueStorage, socketImplementation: socketImplementation)
    }

    init(metadata: AppMetadata, projectId: String, relayHost: String, logger: ConsoleLogging, kms: KeyManagementService, keyValueStorage: KeyValueStorage, socketImplementation: WebSocketConnecting.Type) {
        self.metadata = metadata
        self.logger = logger
        self.kms = kms
        let relayClient = RelayClient(relayHost: relayHost, projectId: projectId, keyValueStorage: keyValueStorage, socketImplementation: socketImplementation, logger: logger)
