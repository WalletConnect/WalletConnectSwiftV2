...
class NonControllerSessionStateMachineTests: XCTestCase {
    ...
    func testUpdateMethodsPeerSuccess() {
        ...
        networkingInteractor.requestPublisherSubject.send((session.topic, RPCRequest.stubUpdateNamespaces(), Date(), nil))
...
//    }
    ...
    func testUpdateMethodPeerErrorSessionNotFound() {
        networkingInteractor.requestPublisherSubject.send(("", RPCRequest.stubUpdateNamespaces(), Date(), nil))
        usleep(100)
    ...
    func testPeerUpdateExpirySuccess() {
        ...
        networkingInteractor.requestPublisherSubject.send((session.topic, RPCRequest.stubUpdateExpiry(expiry: twoDaysFromNowTimestamp), Date(), nil))
    ...
    func testPeerUpdateExpiryTtlTooHigh() {
        ...
        networkingInteractor.requestPublisherSubject.send((session.topic, RPCRequest.stubUpdateExpiry(expiry: tenDaysFromNowTimestamp), Date(), nil))
    ...
    func testPeerUpdateExpiryTtlTooLow() {
        ...
        networkingInteractor.requestPublisherSubject.send((session.topic, RPCRequest.stubUpdateExpiry(expiry: oneDayFromNowTimestamp), Date(), nil))
        let potentaillyExtendedSession = storageMock.getAll().first {$0.topic == session.topic}!
