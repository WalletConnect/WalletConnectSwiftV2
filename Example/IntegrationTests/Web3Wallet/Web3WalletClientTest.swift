...
final class Web3WalletClientTests: XCTestCase {
    var dapp: Web3ClientDelegate!
    var wallet: Web3ClientDelegate!

    static private func makeClientDelegate(name: String) -> Web3ClientDelegate {
        ...
        let client = Web3WalletClientFactory.create(
        ...
        return Web3ClientDelegate(client: client)
    }
    ...
    func testSessionPropose() async throws {
        ...
        let uri = try await dapp.client.signClient.connect(requiredNamespaces: requiredNamespaces)
        try await wallet.client.signClient.pair(uri: uri!)
    ...
    func testSessionReject() async throws {
        ...
        let uri = try await dapp.client.signClient.connect(requiredNamespaces: ProposalNamespace.stubRequired())
        try await wallet.client.signClient.pair(uri: uri!)
    ...
    func testSessionPing() async throws {
        ...
        dapp.onSessionSettled = { sessionSettled in
            Task(priority: .high) {
                try! await self.dapp.client.signClient.ping(topic: sessionSettled.topic)
            }
        }
    ...
    func testSessionRequest() async throws {
        ...
        dapp.onSessionSettled = { [unowned self] settledSession in
            Task(priority: .high) {
                ...
                try await dapp.client.signClient.request(params: request)
            }
        }
    ...
    func testNewSessionOnExistingPairing() async {
        ...
        dapp.onSessionSettled = { [unowned self] _ in
            dappSettlementExpectation.fulfill()
            let pairingTopic = dapp.client.signClient.getPairings().first!.topic
            if !initiatedSecondSession {
                Task(priority: .high) {
                    _ = try! await dapp.client.signClient.connect(requiredNamespaces: requiredNamespaces, topic: pairingTopic)
                }
